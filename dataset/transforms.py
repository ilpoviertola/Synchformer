...
def make_class_grid(
    ...
    if add_extreme_offset:
        ...
        grid = torch.cat(
            [grid, torch.tensor([extreme_value])]
    ...
    def forward(self, item):
        ...
        if self.do_offset:
            ...
            if offset_sec is None and v_start_i_sec is None:
                ...
                assert (
                    v_start_max_sec > 0
                ), f'{v_len_frames} {v_crop_len_frames} {v_fps} @ {item["path"]}'
                ...
                assert (
                    0 <= v_start_sec <= v_start_max_sec
                ), f'{v_start_sec} {v_start_max_sec} {item["path"]}'
    ...
    def forward(self, item):
        ...
        if self.do_offset:
            ...
            if offset_sec is None and v_start_i_sec is None:
                ...
                offset_is_syncable = (
                    random.random() < self.prob_syncable
                )  # 1=syncable, 0=non-syncable
...
class PadOrTruncate(torch.nn.Module):
    ...
    def __init__(
        self, max_spec_t: int, pad_mode: str = "constant", pad_value: float = 0.0
    ):
        super().__init__()
