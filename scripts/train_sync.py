...
def train(cfg):
    ...
    assert hasattr(
        cfg, "ckpt_path"
    ), f"No ckpt_path in the config: {cfg} for worker {global_rank}"
    ...
    if cfg.training.run_test_only or cfg.training.resume or cfg.training.finetune:
        ...
        if cfg.training.finetune:
            ...
            _ckpt_load_status = model_without_ddp.load_state_dict(
            ...
            logging.info(
                f'Finetuning from: {ckpt_cfg.ckpt_path} on {datasets["train"].__class__.__name__}'
            )
            ckpt_metrics[cfg.training.metric_name] = (
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in phases_to_run_on:
            ...
            for it in range(iter_times):
                ...
                ids_to_cache = set(
                    torch.arange(0, total_samples, total_samples / log_max_items)
                    .long()
                    .tolist()
                )
                ...
                for i, batch in enumerate(loaders[phase]):
                    ...
                    if iter_step == 0 and phase in ["train", "valid"]:
                        if is_master(global_rank):
                            logger.vizualize_input(
                                vid, aud, batch, iter_step, phase, cfg
                            )
                    ...
                    with torch.autocast(
                        ...
                        with torch.set_grad_enabled(phase == "train"):
                            loss, logits = model(
                                vid, aud, targets[target_key], loss_fn=loss_fn
                            )
                    ...
                    if not torch.isfinite(
                        loss
                    ).all():  # `.all()` just for the sake of code consistency
                        logging.error(f"Loss is not finite: {loss}")
                        raise RuntimeError(
                            f"Worker (#{global_rank}): Loss is not finite: {loss}"
                        )
                    ...
                    iter_results = dict(
                        ...
                        targets=[
                            apply_fn_recursive(targets[target_key], lambda x: x.cpu())
                        ],
                        path_suffix=[
                            str(Path(p).relative_to(datasets[phase].vids_dir))
                            for p in batch["path"]
                        ],
                    )
                    losses_m["loss_total"].update(loss.item(), len(vid))
                    if "offset_sec" in targets:
                        iter_results["offset_sec"] = [
                            apply_fn_recursive(targets["offset_sec"], lambda x: x.cpu())
                        ]
                    ...
                    if is_master(global_rank) and i % cfg.logging.log_frequency == 0:
                        ...
                        logger.log_iter_loss(
                            iter_loss, iter_step, phase, prefix="total"
                        )
                        if phase == "train":
                            logger.add_scalar(
                                "lr", lr_scheduler.get_last_lr()[0], iter_step
                            )
                        if cfg.logging.get("vis_segment_sim", False) and phase in [
                            "train",
                            "valid",
                        ]:
                            ...
                            if i % (cfg.logging.log_frequency * 10) == 0:
                                ...
                                logger.vizualize_segment_sim(
                                    cfg, model, vid, aud, iter_step, phase
                                )
                                if phase == "train":
                        ...
                        msg += (
                            f" Data (t): {data_time_m.val:.3f} ({data_time_m.avg:.3f})"
                        )
                        msg += f" Batch (t): {batch_time_m.val:.3f} ({batch_time_m.avg:.3f},"
                        msg += (
                            f" {samples_per_s:#g}/s, {samples_per_s_per_gpu:#g}/s/gpu)"
                        )
                    ...
                    if i in ids_to_cache:
                        for res_k, res_v in iter_results.items():
                            ...
                            running_results[res_k] = (
                                running_results.get(res_k, default_obj)
                                + iter_results[res_k]
                            )
            ...
            if phase == cfg.training.early_stop_phase:
                has_model_improved = early_stopper.is_new_model_better_than_curr(
                    metrics
                )
    ...
    ckpt_epoch, best_metrics_val = load_ckpt(
    ...
    if is_master(global_rank):
        ...
        shutil.copyfile(
            str(p),
            str((p.parent / f"{p.stem}_e{ckpt_epoch}_t{curr_t}").with_suffix(p.suffix)),
        )
    if dist.is_initialized():
        dist.barrier()
    ...
    for it in range(iter_times):
        ...
        for i, batch in enumerate(loaders[phase]):
            ...
            for res_k, res_v in iter_results.items():
                default_obj = [] if isinstance(res_v, list) else 0
                running_results[res_k] = (
                    running_results.get(res_k, default_obj) + iter_results[res_k]
                )
