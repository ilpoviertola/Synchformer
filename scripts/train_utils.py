...
def get_datasets_v2(cfg, which_datasets=["train", "valid", "test"]):
    ...
    for phase in which_datasets:
        ...
        ts_cfgs = (
            ...
            if phase == "train"
            else cfg.transform_sequence_test
        )
        ts = (
            [lambda x: x]
            if ts_cfgs is None
            else [instantiate_from_config(c) for c in ts_cfgs]
        )
...
def get_batch_sizes(cfg, num_gpus=None):
    return {
        "train": cfg.training.base_batch_size,
        "test": cfg.training.base_batch_size,
    }
...
def load_ckpt(cfg, model_wo_ddp, optimizer=None, scaler=None, lr_scheduler=None):
    ...
    if not cfg.training.finetune:
        ...
        if (
            ...
            logging.warning(
                "Check if the above missing keys are expected. If so, ignore this warning."
    ...
    if cfg.training.finetune:
        ...
        new_vid_dir = (
            f"{Path(cfg.data.vids_path).parent.stem}/{Path(cfg.data.vids_path).stem}"
        )
        ...
        if ckpt_cfg.data.dataset.target == cfg.data.dataset.target:
            logging.warning(
                f"Finetuning on the same dataset: {ckpt_cfg.data.dataset.target}"
            )
        logging.info(
...
def verbose_epoch_progress(
    ...
    if is_master(global_rank):
        ...
        if "loss_off" in running_results:
            logger.log_epoch_loss(
                running_results["loss_off"], epoch, phase, prefix="off"
            )
        if "loss_doubt" in running_results:
            logger.log_epoch_loss(
                running_results["loss_doubt"], epoch, phase, prefix="doubt"
            )
    ...
    is_task_binary = (
        logits.shape[-1] == 2
    )  # during fine-tuning for syncability the task is binary
    if is_task_binary:
        if is_master(global_rank):
        ...
        off_transf = get_transform_instance_from_compose(
            train_dataset.transforms, "TemporalCropAndOffset"
        )
        ...
        label2metrics = calc_performance_per_class(
            target2label, "off", logits, targets, train_dataset=train_dataset
        )
        ...
        k = min(
            logits.shape[-1], 5
        )  # top-5 is not always available (e.g. for 3 classes)
        if not all(["accuracy_1" in v for v in label2metrics.values()]):
            logging.warning(
                "Not all offset classes have predictions. Median accs of those are replaced with 0.0"
            )
        metrics["accuracy_1_median"] = np.median(
            [v.get("accuracy_1", 0.0) for v in label2metrics.values()]
        )
        metrics[f"accuracy_{k}_median"] = np.median(
            [v.get(f"accuracy_{k}", 0.0) for v in label2metrics.values()]
        )

        if is_master(global_rank):
            plots_and_log_perf_per_cls(
                logger, phase, epoch, train_dataset, label2metrics, targets, preds
            )
            logger.log_epoch_metrics(metrics, epoch, phase)
...
def plot_n_log_perf(
    logger, phase, epoch, train_dataset, oos_targets, off_preds_sec, off_targets
):
    # make plots: 2 x 3:
    ...
    fig.suptitle(
        f"{logger.start_time} | {phase} (e{epoch}) | {train_dataset.__class__.__name__}"
    )
    ...
    ax[0, 0].scatter(
        off_targets_ins, (off_preds_sec_4ins - off_targets_ins) ** 2, s=1, alpha=0.5
    )
    ...
    ax[1, 0].scatter(
        off_targets_oos, (off_preds_sec_4oos - off_targets_oos) ** 2, s=1, alpha=0.5
    )
...
# NOTE: wrap the call in `is_master` to avoid calling `plt.show()` on all processes
def plots_and_log_perf_per_cls(
    logger,
    phase,
    epoch,
    train_dataset,
    ...
    _dict = {
        "perf_per_cls": perf_list,
        "preds": pred_count_list,
        "targets": target_count_list,
    }
    for i, (name, vals) in enumerate(_dict.items()):
        ...
        for rect in ax[i].barh(
            y, vals, width, alpha=0.5, color="C0", lw=1, align="center"
        ):
            ax[i].annotate(
                f"{rect.get_width():.2f}",
        ...
        ax[i].autoscale(
            enable=True, axis="y", tight=True
        )  # tight layout inside of the figure
...
def calc_performance_per_class(
    target2label,
    class_category,
    logits_off,
    targets_off,
    ...
    target_cls2preds = {
        target_cls: {"logits_off": [], "targets_off": []}
        for target_cls in target2label.keys()
    }
    ...
    for logits_targets_i in zip(*logits_and_targets):
        if class_category == "off":
            ...
            raise NotImplementedError(
                "performance per data class is not implemented (see running_results)"
            )
    ...
    for key, preds in target_cls2preds.items():
        if len(preds["logits_off"]) > 0 and len(preds["targets_off"]) > 0:
            ...
            metric_name2val = calc_cls_metrics(
                torch.tensor(preds["targets_off"]),
                torch.stack(preds["logits_off"]),
                only_accuracy=True,
                verbose=False,
                add_doubt_cls=add_doubt_cls,
            )
        else:
...
def verbose_test_progress(global_rank, logger, cfg, running_results, ckpt_epoch):
    ...
    if is_master(global_rank):
        logits = torch.cat(running_results["logits"]).float()
        targets = torch.cat(running_results["targets"]).long()
        metrics = calc_cls_metrics(
            targets, logits, add_doubt_cls=cfg.training.get("add_doubt_cls", False)
        )
...
def calc_cls_metrics(
    ...
    calc_tol_accuracy=True,
    softmaxed_outputs=False,
    calc_pr_rec_f1=False,
):
    """
    ...
    unique_preds = sorted(
        list(set(preds[:, 0].tolist()))
    )  # picking only the top prediction
    if verbose and (len(unique_preds) < num_cls):
        logging.warning(
            f"Some classes never occured in _outputs_. {prefix} pred classes: {unique_preds}"
        )
    ...
    for k in topk:
        if num_cls == 2:  # binary classification
            if k == 2:  # silence the warning
                continue
            metrics_dict[f"{prefix}accuracy_{k}"] = top_k_accuracy_score(
                targets, targets_pred[:, 1], k=k, labels=range(num_cls)
            )
        else:
            metrics_dict[f"{prefix}accuracy_{k}"] = top_k_accuracy_score(
                targets, targets_pred, k=k, labels=range(num_cls)
            )
    ...
    if calc_tol_accuracy:
        ...
        if add_doubt_cls:
            num_off_cls = (
                num_cls - 1
            )  # we don't need conf cls for metrics with tolerance
        ...
        targets_for_acc_w_tol = torch.stack(
            [targets_left_tol, targets_as_preds, targets_right_tol]
        )
        ...
        for k in topk:
            ...
            metrics_dict[f"{prefix}accuracy_{k}_tol1"] = TPs_w_tol / (
                len(correct_for_maxtopk_w_tol) + 1e-7
            )

        if (
            num_off_cls == 3 and dataset_size > 100
    ...
    if len(unique_targets) < num_cls:
        logging.warning(
            f"Some classes never occured in targets. {prefix} target classes: {unique_targets}"
        )
    ...
    # one-vs-rest
    avg_p = [
        average_precision_score(targets_1hot[:, c], targets_pred[:, c], average=None)
        for c in range(num_cls)
    ]
    roc_aucs = [
        roc_auc_score(targets_1hot[:, c], targets_pred[:, c], average=None)
        for c in range(num_cls)
    ]
    ...
    if calc_pr_rec_f1:
        metrics_dict[f"{prefix}precision"] = precision_score(
            targets, preds[:, 0], zero_division=0.0
        )
        metrics_dict[f"{prefix}recall"] = recall_score(
            targets, preds[:, 0], zero_division=0.0
        )
