...
def cfg_sanity_check_and_patch(cfg):
    if cfg.action == "train_avclip":
        assert cfg.model.params.afeat_extractor.params.get(
            "add_global_repr"
        ) == cfg.model.params.vfeat_extractor.params.get(
            "add_global_repr"
        ), "add_global_repr is diff for A&V"
        assert cfg.model.params.afeat_extractor.params.get(
            "max_segments"
    ...
    assert not (
        cfg.training.resume and cfg.training.finetune
    ), "it is either funetuning or resuming"
    assert not (
        cfg.training.resume and cfg.training.run_test_only
    ), "it is either resuming or testing-only"
    assert not (
        cfg.training.finetune and cfg.training.run_test_only
    ), "it is either finetune or testing-only"

    offset_type = cfg.data.get("offset_type", None)
    if False:  # offset_type is not None:
        if "grid" in offset_type and "loss_fn" in cfg.training:
            assert (
                "mse" not in cfg.training.loss_fn[1]
            ), f"to class but loss: {cfg.training.loss_fn[1]}"
        elif "uniform" in offset_type:
            assert (
                "cross_entropy" not in cfg.training.loss_fn[1]
            ), f"reg but loss: {cfg.training.loss_fn[1]}"
    ...
    if cfg.training.resume:
        assert Path(cfg.logging.logdir, cfg.start_time).exists(), Path(
            cfg.logging.logdir, cfg.start_time
        )
    afeat_extractor = cfg.model.params.afeat_extractor.target
    vfeat_extractor = cfg.model.params.vfeat_extractor.target
    if afeat_extractor.endswith("ResNet18AudioFeatures") and vfeat_extractor.endswith(
        "S3DVisualFeatures"
    ):
        assert (
            cfg.logging.vis_segment_sim is False
        ), "logger.vizualize_segment_sim mults pre-proj features"
...
def get_fixed_off_fname(data_transforms, split):
    ...
    for t in data_transforms.transforms:
        if hasattr(t, "class_grid"):
            ...
            crop_len_sec = (
                int(t.crop_len_sec)
                if t.crop_len_sec == int(t.crop_len_sec)
                else t.crop_len_sec
            )
...
def show_cfg_diffs(a, b, save_diff_path=None):
    ...
    if save_diff_path is None:
        for line in difflib.unified_diff(
            a, b, fromfile="old", tofile="new", lineterm=""
        ):
            print(line)
    else:
