...
class Synchformer(torch.nn.Module):
    ...
    def forward(
        ...
        logits = self.transformer(
            vis, aud
        )  # (B, cls); or (B, cls) and (B, 2) if DoubtingTransformer

        loss = self.compute_loss(
            logits, targets, loss_fn
        )  # (B,); or a tuple of (B,) and (B,)
    ...
    def load_state_dict(self, sd: Mapping[str, Any], strict: bool = True):
        ...
        if "transformer.pos_emb_cfg.pos_emb" in sd:
            ...
            if weight_len > self_len:
                sd["transformer.pos_emb_cfg.pos_emb"] = sd[
                    "transformer.pos_emb_cfg.pos_emb"
                ][:, :self_len, :]
                logging.warning(
                    f"Trimming the state dict for pos emb from {weight_len} to {self_len}"
                )
            elif weight_len < self_len:
                raise ValueError(
                    f"Cant load state dict with shorter seq len ({weight_len} vs {self_len})"
                )
...
class GlobalTransformerWithSyncabilityHead(GlobalTransformer):
    ...
    def __init__(
        ...
        self.off_head = (
            torch.nn.Identity()
        )  # this class is used only during ftuning so this is not needed
